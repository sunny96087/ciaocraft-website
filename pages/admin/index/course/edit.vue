...
async function getCourseData() {
  try {
    ...
    const res = await store.apiGetAdminCourse(data)
    const result = res.data
    console.log(`getCourseData result = ${JSON.stringify(result)}`)
    if (result.statusCode === 200) {
      courseInfo.value = result.data
      console.log(`courseInfo = ${JSON.stringify(courseInfo.value)}`)

      // 轉換課程項目
      courseInfo.value.courseItems = result.data.courseItemId.map(
        (item: {
          _id: string
          courseId: string
          capacity: number
          ...
          mainCourseName: item.mainCourseName,
      ...
      // teacherId 是物件，將其值設定為 _id 的值
      if (courseInfo.value.teacherId && typeof courseInfo.value.teacherId === 'object') {
        courseInfo.value.teacherId = courseInfo.value.teacherId._id
      }
    } else {
...
// NOTE 編輯課程 (Back)
async function editCourse(courseStatus: number) {
  // if (
  ...
  try {
    ...
    const res = await store.apiUpdateCourse(data)
    const result = res.data
...
<template>
  <div class="">
    ...
    <div class="admin-breadcrumbs">
      ...
      <span class="text-dark1">編輯課程</span>
    </div>

    <!-- * title -->
    <div class="admin-page-title">編輯課程</div>
        ...
        <div class="row-input">
          ...
          <div class="admin-radio-row">
            <label class="admin-radio-row-label">
              <input type="radio" value="0" v-model.number="courseInfo.courseTerm" />單堂體驗
            </label>
            <label class="admin-radio-row-label">
              <input type="radio" value="1" v-model.number="courseInfo.courseTerm" />培訓課程
            </label>
          </div>
        </div>
        ...
        <div class="row-input">
          ...
          <div class="flex flex-col">
            ...
            <div
              ...
              v-if="courseInfo.courseItems && courseInfo.courseItems.length"
            >
              <div
    ...
    <div class="mt-[28px] flex gap-3">
      <button class="btn-orange-outline" @click="editCourse(0)">儲存先不上架</button>
      <button class="btn-orange" @click="editCourse(1)">上架課程</button>
    </div>
  </div>
</template>
<style scoped>
